...
public class playerScript : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        spawnPoint = gun.transform.Find("Bullet Spawnpoint").gameObject;
    ...
    void Update()
    {
        ...
        if (Physics.Raycast(spawnPoint.transform.position, spawnPoint.transform.forward, out hit, 1000))
        {
            ...
            //if the raycast hits any game object where its name contains "alien" and we aren't already shooting we will start the shooting coroutine
            if (hit.collider.name.Contains("Alien"))
            {
                if (!isShooting)
                {
